...
function addToCartFromPage(element) {
    ...
    let price = parseFloat(document.getElementsByClassName('mkdf-single-product-summary')[0].getElementsByTagName('ins')[0].getElementsByClassName('woocommerce-Price-amount')[0].firstChild.data);
    price = parseFloat(price);
    let name = document.getElementsByClassName('mkdf-single-product-title')[0].innerText;
    let params = { content_name: name, content_type: 'product', content_ids: [prodId], value: price, currency: "EUR" };
    fbq('track', 'AddToCart', params);
